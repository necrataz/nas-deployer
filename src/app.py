# ==============================================================================
# NAS Deployer v2.0.7.1 - 主 GUI (ttkbootstrap)
# 新增: VPN 卡片启停开关 (启动/停止按钮按状态切换) + 自动跳过已装容器
# v2.0.7.1: daemon.json 加 3 个 registry-mirrors + 重启 docker + 验证生效
# ==============================================================================
...
class NASDeployerApp:
    ...
    def _open_navigation_page(self):
        """v2.0: 打开 NAS 服务导航聚合页"""
        if not self.connection.is_connected():
            messagebox.showerror("错误", "请先连接 NAS")
            return
        # 已安装 = 当前 selected_apps（用户刚装过的）
        # 更准确的来源: 读 self.app_checkboxes (用户勾选的也算"想用的")
        # 但"已安装"是真实跑着的容器, 我们用状态 Tab 的列表
        try:
            from navigation import open_navigation_page
            # 从 app.py 状态 Tab 拿 (如果有缓存), 或实时拉
            containers = self.connection.get_installed_containers()
            installed = [c["name"] for c in containers if c.get("name")]
            if not installed:
                messagebox.showinfo("提示", "暂无已安装服务 (状态 Tab 是空的)")
                return
            nas_name = "NAS"
    ...
    def _show_credentials_dialog(self):
        ...
        try:
            ...
            top.minsize(480, 400)  # v2.0.4.2: 最小尺寸保护
            ...
            # v2.0.4.2: 自适应尺寸 — 凭据长度不一, 算实际需要的高度
            top.update_idletasks()
            req_w = top.winfo_reqwidth()
            req_h = top.winfo_reqheight()
            screen_w = top.winfo_screenwidth()
            screen_h = top.winfo_screenheight()
            final_w = min(max(req_w, 600), screen_w - 100)
            final_h = min(max(req_h, 500), screen_h - 100)
            x = (screen_w - final_w) // 2
            y = (screen_h - final_h) // 2
            top.geometry(f"{final_w}x{final_h}+{x}+{y}")
    ...
    def _build_status_tab(self):
        ...
        # v2.0: 一键导航聚合按钮 (显眼位置)
        ttk.Button(btn_frame, text="🧭 打开服务导航", command=self._open_navigation_page, bootstyle="success").pack(side=LEFT, padx=5)
        # v2.0.5: 卸载按钮 (显眼位置)
        ttk.Button(btn_frame, text="🗑 卸载选中", command=self._uninstall_checked_apps, bootstyle="danger-outline").pack(side=RIGHT, padx=5)

        # v2.0.5.1: 勾选面板 (可独立操作, 不需 treeview 选中)
        from uninstall_panel import UninstallPanel
        self.uninstall_panel = UninstallPanel(
            tab, on_uninstall=self._uninstall_apps_from_panel,
            on_select_change=self._on_panel_selection_change,
        )
        self.uninstall_panel.frame.pack(fill=X, pady=(0, 5))
        ...
        # v2.0: 右键容器名 → 直接跳到该容器服务页
        self.tree_menu.add_command(label="🧭 在浏览器打开此服务", command=self._open_service_in_browser)
        self.tree_menu.add_separator()
        # v2.0.5: 卸载单个容器
        self.tree_menu.add_command(label="🗑 卸载此服务 (保留数据)", command=self._uninstall_single_container)
        self.tree_menu.add_command(label="🗑💥 卸载此服务 (删数据!)", command=self._uninstall_single_container_with_volumes)
    ...
    def _ask_mihomo_subscription(self) -> Optional[str]:
        ...
        Returns:
            str URL    : 用户填了, 装时写进 config.yaml
            "" (空串)  : 用户选"暂不填, 装默认占位"
            None        : 用户取消 (整个安装流程取消)

        v2.0.4.2: 自适应窗口尺寸 — 不需拖拽
        """
        top = tk.Toplevel(self.root)
        top.title("🌐 配置 mihomo 代理订阅")
        # v2.0.4.2: 先给一个保守尺寸, update 后再自适应
        top.geometry("560x320")
        top.minsize(480, 280)
        top.transient(self.root)
        top.grab_set()  # 模态

        result = {"value": "cancel"}  # 默认取消

        # 标题
        ttk.Label(
            top, text="🧭 mihomo 订阅配置",
            font=("Helvetica", 14, "bold")
        ).pack(pady=(15, 5))
        ttk.Label(
            top,
            text="粘贴你的代理订阅 URL (clash / clash-meta 格式)",
            font=("Helvetica", 10),
        ).pack(pady=(0, 12))

        # 输入框
        entry_frame = ttk.Frame(top, padding=(20, 0))
        entry_frame.pack(fill=X)
        entry = ttk.Entry(entry_frame, font=("Consolas", 10))
        ...
        ttk.Label(
            top,
            text="格式示例: https://your-provider.com/api/v1/client/subscribe?token=xxx\n"
                 "空着点'暂不填' → 容器能起但无代理节点",
        ...
        btns = ttk.Frame(top, padding=(20, 0))
        btns.pack(fill=X)
        ttk.Button(btns, text="✅ 确定", command=on_ok, bootstyle="success", width=12).pack(side=LEFT, padx=2)
        ttk.Button(btns, text="⏭ 暂不填", command=on_skip, bootstyle="secondary-outline", width=12).pack(side=LEFT, padx=2)
        ttk.Button(btns, text="❌ 取消安装", command=on_cancel, bootstyle="danger-outline", width=12).pack(side=RIGHT, padx=2)

        # 回车 = OK, Esc = 取消
        top.bind("<Return>", lambda e: on_ok())
        top.bind("<Escape>", lambda e: on_cancel())

        # v2.0.4.2: 自适应尺寸 — update_idletasks 算实际需要, 再调整
        # 1. 先算出窗口"自然需要"的尺寸 (不强制 geometry)
        top.update_idletasks()
        req_w = top.winfo_reqwidth()
        req_h = top.winfo_reqheight()
        # 2. 屏幕边界保护 (用户可能是 4K + DPI 缩放)
        screen_w = top.winfo_screenwidth()
        screen_h = top.winfo_screenheight()
        final_w = min(max(req_w, 560), screen_w - 100)
        final_h = min(max(req_h, 320), screen_h - 100)
        # 3. 居中
        x = (screen_w - final_w) // 2
        y = (screen_h - final_h) // 2
        top.geometry(f"{final_w}x{final_h}+{x}+{y}")

        top.wait_window()  # 阻塞到 destroy

        if result["value"] == "cancel":
            return None
        return result["value"]

    def _install_thread(self, apps, mihomo_sub=None):
        ...
        try:
            ok, msg = self.connection.install_apps_streaming(
                ...
                on_progress=progress.update_progress,
                is_cancelled=progress.is_cancelled,
                mihomo_subscription_url=mihomo_sub,  # v2.0.4
            )
            progress.finish(ok, msg if ok else f"❌ {msg}")
        except Exception as e:
    ...
    def _refresh_status_thread(self):
        ...
        # v2.0.5.1: 同步刷新卸载勾选面板
        if hasattr(self, "uninstall_panel"):
            self.uninstall_panel.set_containers(containers)
    ...
    # -------------------- v2.0.5 卸载 --------------------
    def _get_selected_container_name(self) -> Optional[str]:
        """从状态 Tab 取选中容器的 name (统一入口)"""
        sel = self.status_tree.selection()
        if not sel:
            return None
        return str(self.status_tree.item(sel[0])["values"][0])

    def _uninstall_single_container(self):
        """v2.0.5: 卸载单个容器 (保留数据)"""
        name = self._get_selected_container_name()
        if not name:
            return
        if not messagebox.askyesno(
            "确认卸载",
            f"将卸载容器 {name}\n\n"
            f"✅ 容器 + 镜像被删除\n"
            f"📁 数据卷保留 (./configs, /photos 等)\n\n"
            f"确认?"
        ):
            return
        self._log(f"=== 卸载 (保留数据): {name} ===")
        threading.Thread(target=self._uninstall_thread, args=([name], False), daemon=True).start()

    def _uninstall_single_container_with_volumes(self):
        """v2.0.5: 卸载单个容器 + 删数据卷 (危险!)"""
        name = self._get_selected_container_name()
        if not name:
            return
        if not messagebox.askyesno(
            "⚠️ 危险操作",
            f"将卸载容器 {name} 并删除所有数据!\n\n"
            f"❌ 容器 + 镜像 + ./configs 数据卷全部删除\n"
            f"⚠️ 你的照片/书/下载会一起没!\n\n"
            f"确认? (此操作不可恢复)"
        ):
            return
        self._log(f"=== 卸载 (删数据!): {name} ===")
        threading.Thread(target=self._uninstall_thread, args=([name], True), daemon=True).start()

    def _uninstall_selected_container(self):
        """v2.0.5: 顶部卸载按钮 — 卸载当前选中 (默认保留数据)"""
        # 默认走单容器卸载, 跟右键菜单走同一条
        self._uninstall_single_container()

    def _uninstall_checked_apps(self):
        """v2.0.5.1: 顶部卸载按钮 — 卸载勾选面板里所有勾选的"""
        if not hasattr(self, "uninstall_panel"):
            return
        checked = self.uninstall_panel.get_checked()
        if not checked:
            messagebox.showinfo("提示", "请先勾选要卸载的服务 (面板里打勾)")
            return
        if not messagebox.askyesno(
            "确认卸载",
            f"将卸载 {len(checked)} 个服务 (保留数据):\n\n"
            + "\n".join(f"  • {n}" for n in checked)
            + "\n\n确认?"
        ):
            return
        self._log(f"=== 批量卸载 (保留数据): {', '.join(checked)} ===")
        threading.Thread(target=self._uninstall_thread, args=(checked, False), daemon=True).start()

    def _uninstall_apps_from_panel(self, names, remove_volumes):
        """v2.0.5.1: panel 自己的卸载按钮回调"""
        if not names:
            return
        if remove_volumes:
            prompt = (
                f"⚠️ 危险操作\n\n"
                f"将卸载 {len(names)} 个服务并删除所有数据!\n\n"
                f"❌ 容器 + 镜像 + ./configs 数据卷全部删除\n"
                f"⚠️ 你的照片/书/下载会一起没!\n\n"
            ...
            prompt = (
                f"将卸载 {len(names)} 个服务:\n\n"
                + "\n".join(f"  • {n}" for n in names)
                + "\n\n✅ 数据卷保留 (./configs, /photos 等)\n确认?"
            )
        if not messagebox.askyesno("确认卸载" if not remove_volumes else "⚠️ 危险操作", prompt):
            return
        self._log(f"=== 批量卸载 ({'删数据' if remove_volumes else '保留数据'}): {', '.join(names)} ===")
        threading.Thread(target=self._uninstall_thread, args=(names, remove_volumes), daemon=True).start()

    def _on_panel_selection_change(self, selected_count):
        """v2.0.5.1: panel 勾选变化回调 — 更新顶部按钮状态"""
        # 可以将来扩展: 选中 0 个时禁用按钮, >0 个时启用
        pass

    def _uninstall_thread(self, apps, remove_volumes):
        """v2.0.5: 卸载后台线程 — 带进度窗口"""
        progress = ProgressWindow(self.root, title="卸载进度")
        try:
            progress.update_progress(30, f"正在卸载 {', '.join(apps)}...")
            ok, msg = self.connection.uninstall_apps(apps, DOCKER_COMPOSE_YML, remove_volumes=remove_volumes)
            progress.finish(ok, f"{'✅' if ok else '❌'} {msg[:500]}")
            self._log(f"{'✅' if ok else '❌'} 卸载\n{msg[:1000]}")
            if ok:
                # 卸载成功 → 自动刷新状态 Tab
    ...
    def _open_service_in_browser(self):
        """v2.0: 右键容器 → 在浏览器打开该服务的 URL"""
        sel = self.status_tree.selection()
        if not sel:
            return
        name = str(self.status_tree.item(sel[0])["values"][0])
        # 拿容器端口（从 Tree 第 4 列 "ports"）
        ports_str = str(self.status_tree.item(sel[0])["values"][3]) if len(self.status_tree.item(sel[0])["values"]) > 3 else ""
        # ports 格式: "0.0.0.0:8080->8080/tcp, 0.0.0.0:8081->8081/tcp"
        import re
        m = re.search(r"0\.0\.0\.0:(\d+)", ports_str)
        if not m:
            messagebox.showinfo("提示", f"容器 {name} 没有暴露端口")
            return
        port = m.group(1)
        # 拼 NAS IP（用当前连接的 host）
        nas_ip = self.connection.host or "127.0.0.1"
        url = f"http://{nas_ip}:{port}"
        try:
            import webbrowser
            webbrowser.open(url)
            self._log(f"🧭 打开 {name}: {url}")
        except Exception as e:
