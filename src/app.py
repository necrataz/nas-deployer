...
class NASDeployerApp:
    ...
    def _pull_thread(self, apps):
        ...
        try:
            progress.update_progress(10, "上传 compose 文件...")
            # 复用 install_apps_streaming 但只跑 pull 阶段
            ...
            progress.update_progress(20, f"拉取镜像 ({', '.join(sorted(profiles_to_pull))})...")
            pull_cmd = f"mkdir -p /tmp/nas_deploy && echo 'docker compose yml placeholder' > /tmp/nas_deploy/docker-compose.yml; cd /tmp/nas_deploy && docker compose {profiles_str} pull"
            # 直接用 streaming
            exit_code = self.connection.run_command_streaming(
                pull_cmd,
                on_line=progress.append_log,
                is_cancelled=progress.is_cancelled,
                timeout=1800,
            ...
            progress.finish(
                exit_code == 0,
                "✅ 拉取完成" if exit_code == 0 else f"❌ 失败 (exit={exit_code})"
            )
        except Exception as e:
            progress.finish(False, f"异常: {e}")
        finally:
